...
workshop cameras by running:
    ...
    $ guild run coco-mask-rcnn-resnet101:collect use-proxy=yes
...
model is listed as:
    ...
    pets-faster-rcnn-resnet101:finetune
...
pets Mask RCNN detector:
    ...
    $ guild run pets-mask-rcnn-resnet101:finetune
...
We'll have a completed run on hand that we can use for detection and
be able to stop training early.

**TODO:** This might be a good place to highlight the pipeline
configuration used in the TF object detection library. That wraps up a
whole ton of hyperparameters and we can use that present that
topic. (garrett)

**TODO:** We can also use the config to highlight the augmentation
that the pipeline uses. The config only specifies image flipping. I've
played around with the other augmentation forms and got
similar-to-worse results. I'd like to take advantage of the fact we'll
be running 20 GPUs (though very slow) and have each pair of students
play around with hyperparams to see how the model improves. This
probably doesn't make sense for the custom dataset (below) given the
extremely small dataset size, which would be poor for validation/test,
I think. (garrett)

**BACKGROUND:** Talk about the role of augmentation

### Evaluate trained model

With our trained model (i.e. with at least one saved checkpoint) we
can evaluate how it does on data it hasn't seen.

To evaluate a model using the pets validation data:

    $ guild run pets-mask-rcnn-resnet101:evaluate

This will generate images with highlighted predicted objects and
output an overal average precision for the model.

We'll use TensorBoard to view the labeled images and precision across
classes.

We can run evaluate on various servers to compare relative performance
based on training checkpoint steps.

**BACKGROUND:** Talk about overfitting, evaluating models, hyperparameters

**TODO:** Hyperparameter discussion might be better placed in the
previous section - see TODO above. (garrett)
...
First export our model:
    ...
    $ guild run pets-mask-rcnn-resnet101:export checkpoint-step=STEP
...
And then detect:
    ...
    $ pets-mask-rcnn-resnet101:detect
...
for `cats-dataset`:
    ...
    $ guild run cats-dataset:collect use-proxy=yes
...
**TODO:** I'm not sure if the label app can support concurrent
labeling - I suspect it cannot and students would be labeling the same
image and end up repeating the same work. It would be great if
everyone could participate here. (garrett)
